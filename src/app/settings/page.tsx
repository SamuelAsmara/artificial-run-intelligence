...
export default async function SettingsPage() {
  /*
   * A coach has one settings page, and it is the coach one — whichever mode
   * they were in when they clicked "Settings". Two screens called Settings
   * were a contradiction, and the athlete screen carried an athlete package
   * that a coach must never end up on. The redirect makes that impossible
   * rather than merely hidden.
   */
  if (await isCoach()) redirect("/coach/settings");
  ...
  const [icuConnection, profile, coach, planScreen, billingPlan] = await Promise.all([
  ...
  return <SettingsView icuConnection={icuConnection} profile={profile} coach={coach} plan={plan} billingPlan={billingPlan} />;
}
