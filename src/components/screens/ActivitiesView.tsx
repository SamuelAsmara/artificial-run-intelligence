...
/**
 * Activities — the athlete's run history: four-week summary tiles, weekly
 * distance and easy-pace trend, and the paged list of every run, with a
 * compare mode for putting runs side by side.
 */

import * as React from "react";
import { useMemo, useState } from "react";
import Link from "next/link";
import { ACT_COPY, fmtPace, type Act, type PacePoint } from "@/lib/screens/activities";
...
/** Stable empty list, so the comparison memos do not rebuild every render. */
const NO_RUNS: ComparableRun[] = [];
...
export function ActivitiesView({
  ...
  const comparable = data.compare ?? NO_RUNS;
  const byId = useMemo(() => new Map(comparable.map((r) => [r.id, r])), [comparable]);
   ...
   *
   * Rendering every matching run would mean several hundred rows for a real
   * history — the page never ends, and finding last Tuesday means scrolling
   * past two years. Fifteen is a screenful.
   */
  ...
  const rows = matching.slice(from, from + PAGE_SIZE).map((a) => ({
    ...a,
    dot: dot[a.type],
    sel: comparing && picked.includes(a.id),
    onPick: comparing
      ? (e: React.MouseEvent) => {
          e.preventDefault();
          toggle(a.id);
  ...
  /*
   * Heart rate is computed on the server, over the runs that have one. `a.hr`
   * is the *display* string — an em dash when no strap was worn — so it is
   * never parsed back into a number here.
   */
  const avgHr = data.avgHr ?? null;

  /*
   * The stats row, on the kit's stat tile, over the window its label promises
   * — the server's four-week summary, not a reduce over every fetched row.
   * `v: null` is the honest reading when there is nothing to average — the
   * tile draws an em dash. It must never be a zero, which an athlete would
   * read as "I ran at 0:00/km".
   */
  ...
  const stats: {
    ...
    {
      id: "runs",
      v: String(summary.runs),
      unit: "runs",
      name: "Completed · 4 weeks",
      icon: STAT_ICONS.distance,
      hue: NUMBERS_HUE.acwr,
    },
    {
      id: "km",
      v: Math.round(summary.totalKm) + "",
      unit: "km",
      name: "Total distance",
    ...
    {
      id: "pace",
      v: summary.avgPaceSec === null ? null : fmtPace(summary.avgPaceSec),
      unit: "/km",
      name: "Average pace",
    ...
    {
      id: "hr",
      v: avgHr === null || !Number.isFinite(avgHr) ? null : String(avgHr),
      unit: "bpm",
    ...
    >
      ...
      >
        ...
        >
          ...
          <Link href="/activities" style={{ color: "var(--color-ink)" }}>
            {copy.navActivities}
          </Link>
      ...
      <section className="grid2">
        <div className="card" style={{ padding: "16px 20px" }}>
          <SectionHeader title={copy.volTitle} hint="last 4 weeks · km" />
          <div style={{ marginBlockStart: "10px" }}>
            <MiniBars
              data={volBars}
              idPrefix="actvol"
              ariaLabel="Weekly distance, last four weeks"
            />
          </div>
        </div>
        <div className="card" style={{ padding: "16px 20px" }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
            <h2 style={{ margin: "0", fontSize: "13px", fontWeight: "600" }}>{copy.paceTitle}</h2>
          ...
          {!hasPaceTrend ? (
            ...
            <svg
              viewBox="0 0 540 96"
              style={{ width: "100%", height: "auto", marginBlockStart: "12px" }}
            >
              {paceGrid.map((g, _i4) => (
              ...
              <path d={paceArea} fill="var(--color-accent)" opacity="0.10" />
              <path d={pacePath} fill="none" stroke="var(--color-accent)" strokeWidth="2" />
              {paceDots.map((d, _i5) => (
                <React.Fragment key={_i5}>
                  <circle cx={d.x} cy={d.y} r="3" fill="var(--color-accent)" />
                </React.Fragment>
              ))}
            </svg>
          )}
      ...
      {comparison ? (
        <ComparePanel
          ...
          copy={{
            title: copy.cmpTitle,
            close: copy.cmpClose,
            efficiency: copy.cmpEff,
            noShape: copy.cmpNoShape,
            axisStart: copy.cmpStartAxis,
            axisFinish: copy.cmpFinishAxis,
            deltaLabel: copy.cmpDelta,
            axisRun: copy.cmpAxisRun,
            axisDist: copy.cmpAxisDist,
            bandTitle: copy.cmpBand,
            bandHr: copy.cmpBandHr,
            hint: copy.cmpChartHint,
            splitsBest: copy.cmpBest,
          }}
        />
      ) : null}
      <section className="card" style={{ padding: "16px 20px" }}>
        <SectionHeader
          title={copy.histTitle}
          style={{ flexWrap: "wrap", rowGap: "10px" }}
          action={
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
              {filters.map((f) => (
                <React.Fragment key={f.name}>
                  <FilterChip active={f.active} onClick={f.pick}>
                    {f.name}
                  </FilterChip>
                </React.Fragment>
              ))}
              {comparable.length >= 2 ? (
                <>
                  ...
                  {autoCandidates.length > 0 ? (
                    <ActionChip
                      icon={CHIP_ICONS.autoPick}
                      onClick={() => setPicked([picked[0], ...autoCandidates.map((r) => r.id)])}
                    >
                      {copy.cmpAuto}
                    </ActionChip>
                  ) : null}
                  <ActionChip
                    armed={comparing}
                    icon={CHIP_ICONS.compare}
                    onClick={() => {
                      setComparing((v) => !v);
                      setPicked([]);
                    }}
                  >
                    {comparing ? copy.cmpExit : copy.cmpStart}
                  </ActionChip>
                </>
              ) : null}
            </div>
        ...
        />
        <div
          className="num actrow"
          style={{
            display: "grid",
            gridTemplateColumns: "64px 1.2fr 1fr 1fr 1fr 1fr 88px",
            gap: "10px",
            ...
            <React.Fragment key={a.id}>
              ...
              >
                ...
                <span style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  ></span>
                  <span style={{ fontSize: "12.5px", fontWeight: "500" }}>{a.name}</span>
                  {a.pb ? (
                    <span
                      className="num"
                      title={copy.pbTitle}
                ...
                <span className="num" style={{ fontSize: "12px", textAlign: "end" }}>
                  {a.km} km
                </span>
                <span
                  className="num hide-m"
                  style={{ fontSize: "12px", color: "var(--color-muted)", textAlign: "end" }}
                >
                  {a.time}
                </span>
                <span
                ...
                >
                  <path d={a.spark} fill="none" stroke={a.sparkColor} strokeWidth="1.4" />
                </svg>
              </Link>
            </React.Fragment>
          ))}
        ...
        {pageCount > 1 ? (
          ...
          >
            <span className="num" style={{ fontSize: "11px", color: "var(--color-faint)" }}>
              {copy.pageRange(
                from + 1,
                Math.min(from + PAGE_SIZE, matching.length),
                matching.length,
              )}
            </span>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              >
                {copy.pageOf(pageIndex + 1, pageCount)}
              </span>
              <button
                className="btn btn-secondary"
                type="button"
