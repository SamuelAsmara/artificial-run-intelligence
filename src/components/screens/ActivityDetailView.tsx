...
export interface ActivityDetailData {
  ...
  streams: { n: number; dist: number[]; vel: number[]; hr: number[]; alt: number[]; time: number[] } | null;
  streamsNote: string | null;
  /** null when nothing was planned for that day — the block is then hidden */
...
export function ActivityDetailView({
  ...
  /** the real run, when one was found; the reference run otherwise */
  data?: ActivityDetailData;
}) {
  // Real streams when the run has them, the reference shape otherwise — so the
  // chart never renders against a different run than the header describes.
  const { s, splits: rawSplits } = useMemo(() => {
    if (data?.streams) return { s: data.streams, splits: splitsFrom(data.streams) };
    return buildStreams();
  }, [data?.streams]);
  ...
  // Header and summary describe the run that was opened. Falling back to the
  // reference copy here would be the bug the athlete reported: clicking one run
  // and reading another.
  const runTitle = data ? `${data.dateLabel} · ${data.distanceKm.toFixed(1)} km` : copy.runTitle;
  const runDate = data ? data.fullDate : copy.runDate;

  const summary = data
    ? [
        { v: data.distanceKm.toFixed(2), unit: "km", name: "Distance" },
        { v: data.duration, unit: "", name: "Moving time" },
        { v: data.pace, unit: "/km", name: "Avg pace" },
        { v: data.avgHr !== null ? String(data.avgHr) : "\u2014", unit: data.avgHr !== null ? "bpm" : "", name: "Avg heart rate" },
        {
          v: data.cardiacDriftPct !== null ? data.cardiacDriftPct.toFixed(1) : "\u2014",
          unit: data.cardiacDriftPct !== null ? "%" : "",
          name: "Cardiac drift",
        },
      ]
    : SUMMARY;
  ...
  /**
   * The planned-vs-actual block, and the band the chart draws for it.
   *
   * A real run uses the session that was actually planned for its date. The
   * reference run keeps the prototype's fixed copy, because that is what it is
   * for. What must never happen is the third case the page used to show: a real
   * run with the prototype's target beside it.
   */
  const cmp = data ? data.comparison ?? null : null;
  const showPva = data ? cmp !== null : true;
  const { pvaLabel, pvaColor, pvaBg, pvaNote } = cmp
    ? { pvaLabel: cmp.label, pvaColor: cmp.color, pvaBg: "var(--color-elevated)", pvaNote: cmp.note }
    : PVA_STATES[paceState];
  const pvaPlanned = cmp ? cmp.plannedLine : copy.pvaPlanned;
  const pvaActual = cmp ? cmp.actualLine : copy.pvaActual;

  // No target pace means no band and no "vs plan" reading. Drawing the
  // prototype's 5:30 line over a run nobody set a pace for is the same lie in
  // a different place.
  const targetPace = data ? cmp?.plannedPaceSec ?? null : PLANNED_PACE;
  const hasTarget = targetPace !== null;

  /* planned-pace band (coach view highlights it) */
  // Derived from the run, not from the fixed range — otherwise the band lands
  // somewhere the pace line never goes.
  const pyOf = (p: number) =>
    Math.min(GEOM.Y1, Math.max(GEOM.Y0,
      GEOM.Y0 + ((p - g.paceAxis.min) / (g.paceAxis.max - g.paceAxis.min)) * (GEOM.Y1 - GEOM.Y0)));
  const tp = targetPace ?? PLANNED_PACE;
  const planY = pyOf(tp).toFixed(1);
  const planYTop = pyOf(tp - 10).toFixed(1);
  const planYH = (pyOf(tp + 10) - pyOf(tp - 10)).toFixed(1);
  const planLabelY = (pyOf(tp - 10) - 4).toFixed(1);
  const planLabel = `PLAN ${fmt(tp)} /km`;
  const chartSub = hasTarget
    ? `Pace axis is inverted — faster is up. Shaded band = planned pace window (${fmt(tp)} ± 10 s).`
    : "Pace axis is inverted — faster is up.";

  /* hover tooltip */
  const hx = hovering ? g.X(hi) : 0;
  const dp = hovering ? Math.round(1000 / vel[hi] - tp) : 0;
