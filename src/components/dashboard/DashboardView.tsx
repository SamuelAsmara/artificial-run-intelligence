...
export interface DashboardData {
  ...
  rail?: {
    ...
    racePrediction?: { text: string; label: string; tone: "positive" | "caution" | "neutral" } | null;
  };
}
...
export function DashboardView({
  ...
  const streakLabel = rail ? (rail.streak > 0 ? `${rail.streak} day streak` : "") : isReal ? "" : copy.streak;
  ...
  const prBand = data?.personalRecords
    ...
    : pbs.map((b) => ({ ...b, href: null as string | null }));
  ...
  const racePredText = rail ? (rail.racePrediction?.text ?? "\u2014") : copy.racePred;
  const racePredLabelText = rail ? (rail.racePrediction?.label ?? "Needs a personal best") : copy.racePredLabel;
  const racePredColor = rail
    ? rail.racePrediction
      ? rail.racePrediction.tone === "positive" ? "var(--color-positive)" : rail.racePrediction.tone === "caution" ? "var(--color-caution)" : "var(--color-ink)"
      : "var(--color-faint)"
    : "var(--color-caution)";
  ...
  const metrics = [
    ...
    { v: (tsbNow >= 0 ? "+" : "") + tsbNow, unit: "", name: "Form (TSB)",
      interp: tsbNow > 20 ? "Very fresh — detraining?" : tsbNow >= 5 ? "Fresh" : tsbNow >= -10 ? "Normal training" : tsbNow >= -30 ? "Carrying load" : "Heavily loaded",
      tone: tsbNow > 20 ? CA : tsbNow >= 5 ? P : tsbNow >= -10 ? M : tsbNow >= -30 ? CA : CA, border: LN },
  ];
  ...
  const METRIC_ICONS: Record<(typeof METRIC_IDS)[number], string> = {
    drift: NB_ICON.drift, volume: NB_ICON.volume, acwr: NB_ICON.ratio, tsb: NB_ICON.form,
  };
