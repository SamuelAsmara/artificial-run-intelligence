...
import { MiniBars, dayCellStyle, SESSION_EDGE, type DayState, type SessionType } from "@/components/ui";
...
export function DashboardView({
  ...
  /*
   * The chat is demo-only, and hidden on a real dashboard.
   *
   * `INITIAL_MSGS` is a scripted exchange — "three hard sessions in 6 days
   * pushed your acute load up 12%", "keeps your ACWR at 1.08" — and
   * `CHAT_REPLIES` cycles canned answers to whatever you type. Every number in
   * it belongs to the prototype's athlete. Presented to a real user it is not a
   * placeholder, it is an AI coach confidently discussing training that never
   * happened.
   *
   * The real explanation already exists and is already honest: "Show
   * reasoning" opens ReasoningPanel, built from this athlete's own numbers.
   * Conversational coaching is a feature to build, not to mock up.
   */
  const showChat = !isReal;
