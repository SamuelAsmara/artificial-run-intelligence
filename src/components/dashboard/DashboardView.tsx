...
export function DashboardView({
  ...
  const pmc = useMemo(() => buildPmc(), []);
  const W = useMemo(() => weeks(), []);
  const segments = useMemo(() => nextSessionSegments(), []);
  const vols = useMemo(() => volumes(), []);
  const { ctlPath, atlPath, tsbPath, ctlArea, tsbArea, pmcGrid, pmcWeeks, pmcEnds } = pmc;

  const [weekView, setWeekView] = useState(3);
  ...
  const statusLabel = score >= 70 ? "Ready to load" : score >= 40 ? "Ease off today" : "Rest day";
  const statusTone = score >= 70 ? P : score >= 40 ? CA : N;
  const RING_C = 2 * Math.PI * 76;
  const ringDash = ((score / 100) * RING_C).toFixed(1) + " " + RING_C.toFixed(1);

  const tsbNow = Math.round(pmc.T[83]);
  const metrics = [
    { v: "2.4", unit: "%", name: "Cardiac Drift", interp: "Low \u2014 good", tone: P, border: LN },
    { v: "42", unit: "km", name: "Weekly Volume", interp: "+12% vs last week", tone: P, border: LN },
    acwrRisk
      ? { v: "1.62", unit: "", name: "ACWR \u00b7 Load Ratio", interp: "Injury risk \u2014 reduce load", tone: N, border: N }
      : { v: "1.08", unit: "", name: "ACWR \u00b7 Load Ratio", interp: "Within safe range", tone: P, border: LN },
  ...
  const sel = selD >= 0 ? wk.days[selD] : null;
  ...
  const weekLabel = wk.label + " \u00b7 " + wk.range;
  const prevWeek = () => { setWeekView(Math.max(0, weekView - 1)); setSelD(-1); };
  const nextWeek = () => { setWeekView(Math.min(11, weekView + 1)); setSelD(-1); };

  /* --- PMC hover --- */
  const Yv = (v: number) => pmc.yT + (1 - (v - pmc.mn) / (pmc.mx - pmc.mn)) * (pmc.yB - pmc.yT);
  const pmcHover = pmcHi >= 0;
  const hx = pmc.x0 + (pmcHi / 83) * (pmc.x1 - pmc.x0);
  const hDate = new Date(new Date(2026, 7, 11).getTime() - (83 - pmcHi) * 86400000);
  ...
  const onPmcMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    setPmcHi(Math.max(0, Math.min(83, Math.round(((fx - 30) / (1150 - 30)) * 83))));
  };
  const onPmcLeave = () => setPmcHi(-1);

  /* --- calendar --- */
  const { calLabel, calCells } = useMemo(() => calendar(calMonth), [calMonth]);
  const calPrev = () => setCalMonth(Math.max(6, calMonth - 1));
  const calNext = () => setCalMonth(Math.min(9, calMonth + 1));
